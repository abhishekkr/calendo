defmodule CalendoWeb.Components.EventType do
  use Phoenix.Component

  import Phoenix.LiveView.Helpers ## for ~H
  import CalendoWeb.LiveViewHelpers

  alias __MODULE__
  alias CalendoWeb.Router.Helpers, as: Routes
  ...
  def calendar(%{
    ...
    prev_month_path = build_path(current_path, %{month: prev_month})
    next_month_path = build_path(current_path, %{month: next_month})

    assigns =
      assigns
      |> assign(prev_month_path: prev_month_path)
    ...
    <div>
      <div class="flex items-center mb-8">
        ...
        <div class="flex justify-end flex-1 text-right">
          <%= live_patch to: @prev_month_path do %>
      ...
      <div class="mb-6 text-center uppercase calendar grid grid-cols-7 gap-y-2 gap-x-2">
        ...
        <%= for i <- 0..(@month_end.day - 1) do %>
          <EventType.day
            index={i}
            current_path={@current_path}
            date={Timex.shift(@month_start, days: i)}
            time_zone={@time_zone} />
        <% end %>
      </div>
  ...
  def time_slot(%{
    ...
    schedule_path = socket
                    |> Routes.live_path(CalendoWeb.ScheduleEventLive, event_type.slug, slot_string)
                    |> URI.decode()
    assigns = assigns
              |> assign(text: text)
